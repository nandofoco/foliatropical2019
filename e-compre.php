...
// $canal = (int) $_POST['canal'];$cliente = $_SESSION['usuario-cod'];
...
$data_retirada = todate(format($_POST['data-retirada']), 'ddmmaaaa');
...
if($retorno) {
	...
	?>
	<script type="text/javascript">
		location.href='<? echo SITE.$link_lang; ?>ingressos/adicionais/';
	</script>
	<?
	exit();
}
...
$link_retorno = SITE.$link_lang.'ingressos/adicionais/';
...
if(is_numeric($cliente) && !empty($forma) && !empty($evento) && ($n_itens > 0)) {
	...
	/*$sql_nome_cliente = sqlsrv_query($conexao_sankhya, "SELECT TOP 1 NOMEPARC, TELEFONE, EMAIL FROM TGFPAR WHERE CODPARC='$cliente' AND CLIENTE='S' AND BLOQUEAR='N' ORDER BY NOMEPARC ASC", $conexao_params, $conexao_options);
	...
	$sql_ins_compra = sqlsrv_query($conexao, "INSERT INTO loja (LO_EVENTO, LO_CLIENTE, LO_PARCEIRO, LO_DATA_COMPRA, LO_DELIVERY, LO_FORMA_PAGAMENTO, LO_CLI_NOME, LO_NUM_ITENS, LO_ORIGEM) VALUES ($evento, $cliente, 54, GETDATE(), $delivery, $forma, '$nome_cliente', $n_itens, 'site')", $conexao_params, $conexao_options);
	$compra = getLastId();
	...
	$sql_tipo_cod = sqlsrv_query($conexao, "SELECT TI_COD, TI_TAG FROM tipos WHERE TI_TAG IN('lounge', 'frisa', 'camarote')", $conexao_params, $conexao_options);
	...
	if($compra > 0) {
		...
		$link_retorno = SITE.$link_lang.'ingressos/pagamento/v2/'.$compra.'/';
		...
		foreach ($_SESSION['compra-site'] as $key => $carrinho) {
			...
			for ($iitem=1; $iitem <=$carrinho['qtde'] ; $iitem++) {
				...
				$sql_item_infos = sqlsrv_query($conexao, "SELECT TOP 1 VE_VALOR, VE_VAGAS, VE_TIPO_ESPECIFICO, VE_VALOR_EXCLUSIVIDADE, VE_DIA, VE_TIPO FROM vendas WHERE VE_COD='".$carrinho['item']."' /*AND ((VE_LOTE IS NOT NULL AND VE_LOTE_ATIVO = 1) OR VE_LOTE IS NULL)*/", $conexao_params, $conexao_options);
				...
				for ($iitemvaga=1; $iitemvaga <= $item_vagas; $iitemvaga++) { 
					...
					//Cadastrar item
					$sql_ins_item = sqlsrv_query($conexao, "INSERT INTO loja_itens (LI_COMPRA, LI_INGRESSO, LI_NOME, LI_ID, LI_VALOR_TABELA, LI_VALOR, LI_EXCLUSIVIDADE, LI_EXCLUSIVIDADE_VAL) VALUES ($compra, ".$carrinho['item'].", '$nome_cliente', $item_id, '$item_valor_tabela','".$carrinho['valor']."', '$excl', '".$exclusividadeval[$key]."')", $conexao_params, $conexao_options);
					$item = getLastId();
								...
								/*	if($delivery && (!$vendas_adicionais_delivery['incluso']) || $vendas_adicionais_opcoes_incluso || ($vendas_adicionais_opcoes_valor > $vendas_adicionais_delivery['valorn'])){
									...
									if($item_tipo != $loja_cod_camarote) {
										...
										if(($vendas_adicionais_nome_exibicao == 'transfer') || ($vendas_adicionais_opcoes_incluso)) {
											...
											$link_retorno = SITE.$link_lang.'ingressos/agendamento/'.$compra.'/';
										}								
									}
		...
		if($delivery) {
			...
			// foreach ($_SESSION['compra-site'] as $delk => $del) { array_push($del_ar, $del['item']); }
			...
			if(sqlsrv_num_rows($sql_vendas_adicionais_delivery) !== false) {
				...
				$link_retorno = SITE.'ingressos/delivery/'.$compra.'/';
		...
		//Atualizar valores	 
		//$sql_update_compra = sqlsrv_query($conexao, "UPDATE TOP (1) loja SET LO_VALOR_PARCIAL='$valor_parcial', LO_VALOR_TOTAL='$valor', LO_VALOR_INGRESSOS='$valor_ingressos', LO_VALOR_ADICIONAIS='$valor_adicionais', LO_DESCONTO='$desconto' WHERE LO_COD='$compra'", $conexao_params, $conexao_options);

		unset($_SESSION['compra-site']);
		...
		if(!($_SERVER['SERVER_NAME'] == "server")){
			...
			if($session_language == 'US') {
				...
				$resposta_texto = "<td align='left' valign='top'><font face='Arial, Helvetica, sans-serif' color='#666666' size='2'>
						...
						Thanks for choosing our company, we wish a carnival 2019 of much revelry!<br /><br />
				...
				$resposta_texto = "<td align='left' valign='top'><font face='Arial, Helvetica, sans-serif' color='#666666' size='2'>
						...
						Obrigado por escolher nossa empresa, desejamos um carnaval 2019 de muita folia!<br /><br />
			...
			$mail->Subject = utf8_decode("Recebemos seu pedido $compra");  //ASSUNTO DA MENSAGEM
		...
		<script type="text/javascript">
			...
			alert("<? echo $lg['compre_adicionais_extra_reserva_confirmada']; ?>");
