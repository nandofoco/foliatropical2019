	...
	<section class="modal-dialog">
		<section class="modal-content">
			<section id="endereco-box">
				<header>
					<h1>Alterar endereço de cobrança</h1>
...
<section id="conteudo">
	...
	if(sqlsrv_num_rows($sql_cliente) > 0) {
		...
		$cliente_celular = trim($cliente['CELULAR']);
		$cliente_sexo = trim($cliente['SEXO']);
		$cliente_origem = utf8_encode($cliente['ORIGEM']);
		...
		<form id="cadastro-cliente" class="cadastro controle" method="post" action="<? echo SITE; ?>clientes/cadastro/post/">
			...
			<section class="secao">
				...
				<p id="cliente-cpfcnpj-box">
					<label for="<? echo ($cliente_pais == 'BR') ? 'cadastro-cpfcnpj' : 'cadastro-passaporte'; ?>"><? echo ($cliente_pais == 'BR') ? ($cliente_pessoa=="J"?"CNPJ":"CPF") : "Passaporte"; ?></label>
					<input type="text" name="<? echo ($cliente_pais == 'BR') ? 'cpfcnpj' : 'passaporte'; ?>" class="input" id="cadastro-<? echo ($cliente_pais == 'BR') ? 'cpfcnpj' : 'passaporte'; ?>" value="<? echo ($cliente_pais == 'BR') ? $cliente_cpfcnpj : $cliente_passaporte; ?>" data-<? echo ($cliente_pais == 'BR') ? ($cliente_pessoa=="J"?"cnpj":"cpf") : "passaporte"; ?>="<? echo ($cliente_pais == 'BR') ? $cliente_cpfcnpj : $cliente_passaporte; ?>">
				</p>
				...
				<p>
					...
					<input type="text" name="ddd" maxlength="2" class="input pequeno" id="cliente-ddd" value="<? echo $cliente_ddd; ?>" />
				</p>
				<p>
				...
				<section class="selectbox ddi pequeno" id="ddi_celular">
					<p><label for="cliente-ddi">DDI Celular:</label></p>
				...
				<p>
					<label for="cliente-ddd-celular">DDD Celular:</label>
					<input type="text" name="ddd_celular" maxlength="2" class="input pequeno" id="cliente-ddd-celular" value="<? echo $cliente_ddd_celular; ?>" />
				</p>
				<p>
...
$(document).ready(function(){
	...
	$("form#cadastro-cliente").find("input[name='origem']").radioSel('<? echo $cliente_origem; ?>');

	$('#cadastro-cliente select[name="pais"]').change(function(event) {
        var pais = $(this).val();
        if(pais=="BR"){
            // console.log($('input[name="pessoa"]:checked').val());
            if($('input[name="pessoa"]:checked').val()=="J"){
            	$('#cliente-cpfcnpj-box').find('label').html('CNPJ:');
            	$('#cliente-cpfcnpj-box').val('');
            	$('#cliente-cpfcnpj-box').find('input').attr('name','cpfcnpj').mask('99.999.999/9999-99');
