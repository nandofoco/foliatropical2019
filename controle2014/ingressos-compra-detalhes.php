...
$q = format($_GET['q']);
if(!empty($q)) $search = is_numeric($q) ? " AND c.COD='$q' " : " AND (c.VALOR LIKE '%$q%' OR t.TI_NOME LIKE '%$q%' OR d.ED_NOME LIKE '%$q%' OR s.ES_NOME LIKE '%$q%') ";

$sql_ingressos = sqlsrv_query($conexao, 
	"DECLARE @compras TABLE(COD INT, TIPO INT, SETOR INT, DIA INT, FILA VARCHAR(255), NIVEL VARCHAR(255), VALOR DECIMAL(10,2), QTDE INT, ESTOQUE INT, BLOCK TINYINT);

	INSERT INTO @compras (COD, TIPO, SETOR, DIA, FILA, NIVEL, VALOR, QTDE, ESTOQUE, BLOCK)
	SELECT MIN(c.CO_COD), c.CO_TIPO, c.CO_SETOR, c.CO_DIA, c.CO_FILA, c.CO_NIVEL, c.CO_VALOR, COUNT(c.CO_COD) AS QTDE, ISNULL(SUM(c.CO_ESTOQUE),0) AS ESTOQUE, MAX(CO_BLOCK) FROM compras c
	WHERE c.CO_EVENTO='$evento' AND c.D_E_L_E_T_=0 
	GROUP BY c.CO_TIPO, c.CO_SETOR, c.CO_DIA, c.CO_FILA, c.CO_NIVEL, c.CO_VALOR

	SELECT c.COD AS CO_COD, c.TIPO AS CO_TIPO, c.SETOR AS CO_SETOR, c.DIA AS CO_DIA, c.FILA AS CO_FILA, c.NIVEL AS CO_NIVEL, c.VALOR AS CO_VALOR, c.QTDE, c.ESTOQUE, c.BLOCK AS CO_BLOCK,
			...
			if($n_ingressos !== false) {
				...
				while($ingressos = sqlsrv_fetch_array($sql_ingressos)) {
					...
					<tr <? if($ingressos_block) { echo 'class="block"'; } ?>>
						...
						<td class="ctrl">
							<a href="<? echo SITE; ?>ingressos/compra/detalhes/<? echo $ingressos_cod; ?>/" class="ver"></a>
				...
				?>
				<tr><td class="nenhum" colspan="8">Nenhum ingresso encontrado</td></tr>
				<?
			}
