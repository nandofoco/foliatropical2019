		...
		<table class="lista tablesorter">
			...
			if($n_blacklist > 0)	 
			{
				...
				while($blacklist = sqlsrv_fetch_array($sql_blacklist)) {
					...
					$sql_cliente = sqlsrv_query($conexao_sankhya, "SELECT TOP 1 NOMEPARC FROM TGFPAR WHERE CODPARC='$bl_usuario'", $conexao_params, $conexao_options);
					...
					if(sqlsrv_num_rows($sql_cliente) > 0) {
						while($loja_cliente_ar = sqlsrv_fetch_array($sql_cliente)) 
						{
							...
							$bl_usuario_nome = $loja_cliente_ar['NOMEPARC'];
					...
					<tr>
						<th>&nbsp;</th>
						<td><? echo $bl_cod; ?></td>
						<td><? echo utf8_encode($bl_usuario_nome); ?></td>
