...
function zeraDescontoFT() {
        $('#desconto_input').val(0) ;
        $('#desconto').html('');
        $('.div-desconto').hide();
        $('.div-desconto-rigth').hide();
        $('#desconto-rigth').html('');
       

        var clone = $( ".valor-total" ).clone();
        $('#valor-total-rodape').html(clone);
    }
...
$(document).ready(function(){	
    ...
    if($('form#cadastro-cliente')[0]) {
        ...
        $('form#cadastro-cliente input[name="pessoa"]:radio').change(function(){
            ...
            var clientecpfcnpj = $('form#cadastro-cliente input[name="cpfcnpj"]');

            switch(pessoa) {
                case 'F':
                    $cpfcnpj.find('label').html('CPF:');
                    $cpfcnpj.find('input[name="cpfcnpj"]:text').unmask().mask('999.999.999-99').val(clientecpfcnpj.data('cpf'));
                ...
                case 'J':
                    ...
                    $cpfcnpj.find('input[name="cpfcnpj"]:text').unmask().mask('99.999.999/9999-99').val(clientecpfcnpj.data('cnpj'));
    ...
    function temporeal() {
        ...
        $.post(site + "include/relatorio-tempo-real.php", {
            ...
            if(resposta.sucesso) {
                /*$("#tempo-real").html("<td class=\"grupo\" colspan=\"3\"><table> \
                    <tr><td>"+resposta.data+"</td><td>"+resposta.qtde_atual+"</td><td>R$ "+resposta.valor_atual+"</td></tr> \
                    <tr><td class=\"especiais\">Pagos</td><td>"+resposta.qtde_pagos+"</td><td>R$ "+resposta.valor_pagos+"</td></tr> \
                    <tr><td class=\"especiais\">Posterior</td><td>"+resposta.qtde_posterior+"</td><td>R$ "+resposta.valor_posterior+"</td></tr> \
                    <tr><td class=\"especiais\">Reservas</td><td>"+resposta.qtde_reservas+"</td><td>R$ "+resposta.valor_reservas+"</td></tr> \
                    <tr><td class=\"especiais\">Aguard. Pgto.</td><td>"+resposta.qtde_aguardando+"</td><td>R$ "+resposta.valor_aguardando+"</td></tr> \
                    </table></td>");*/

                $("<tr class=\"tr-especiais data\"><td class=\"titulo\">"+resposta.data+"</td><td>"+resposta.qtde_atual+"</td><td>R$ "+resposta.valor_atual+"</td></tr> \
                    <tr class=\"tr-especiais\"><td class=\"titulo\">Pagos</td><td>"+resposta.qtde_pagos+"</td><td>R$ "+resposta.valor_pagos+"</td></tr> \
                    <tr class=\"tr-especiais\"><td class=\"titulo\">Posterior</td><td>"+resposta.qtde_posterior+"</td><td>R$ "+resposta.valor_posterior+"</td></tr> \
                    <tr class=\"tr-especiais\"><td class=\"titulo\">Reservas</td><td>"+resposta.qtde_reservas+"</td><td>R$ "+resposta.valor_reservas+"</td></tr> \
                    <tr class=\"tr-especiais\"><td class=\"titulo\">Aguard. Pgto.</td><td>"+resposta.qtde_aguardando+"</td><td>R$ "+resposta.valor_aguardando+"</td></tr>").insertAfter("#tempo-real");                
...
// section#conteudo form#ingresso-compra-novo p input.input#ingresso-estoque {
    ...
    function compras_adicionais_total() {
        ...
        var mostraDescontoFrisa = $('#mostra-desconto-frisa').html();

        if (mostraDescontoFrisa == 'true') {
            ...
            if($('section#conteudo form#compras-adicionais .item-carrinho.frisa')[0]) {
                ...
                if(frisas_qtde > 0){
                    ...
                    $('#valor-desconto-frisa').html(frisas_total);
        ...
        // Desconto 10% - Folia Tropical

            var desconto = $('#desconto_input').val();

            var entraDesconto = 'false';

            var desconto_porcentagem = parseFloat($('#desconto_porcentagem').val()) / 100;

            if (desconto > 0) {
                var novodesconto = total * desconto_porcentagem;
                var subtotal = total - novodesconto;

                $('#desconto_input').val( novodesconto) ;

                novodesconto = novodesconto.toFixed(2).replace(".",",");
                if(novodesconto.length > 6) novodesconto = novodesconto.replace(/([0-9]{3}),([0-9]{2}$)/g, ".$1,$2");


                subtotal = subtotal.toFixed(2).replace(".",",");
                if(subtotal.length > 6) subtotal = subtotal.replace(/([0-9]{3}),([0-9]{2}$)/g, ".$1,$2");

                $('#desconto').html('R$ '+ novodesconto + ' <a href="javascript:void(0)" onclick="zeraDescontoFT()" class="excluir confirm">&times;</a>');
                $('#subtotal').html('R$ '+ subtotal);


                $('#valor-total-rodape').html('R$ '+ subtotal);
    ...
    if($('section#conteudo.multiplo')[0]) {
        ...
        function multiplo_total(adicionar) {
            ...
            if(adicionar > 0) {
                ...
                total += adicionar;
                return ((total.toFixed(2)) <= limite) ? true : false;

            } else if((total.toFixed(2)) <= limite) {
                // totalf = total.toFixed(2);
                // totalf = totalf.replace('.', ',');

                totalf = numberToReal(total);
                $multiplo.find('header.titulo .valor-total span').html(totalf);

                if((total.toFixed(2)) == parseFloat(limite)) {
