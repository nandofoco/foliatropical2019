...
$novo_desconto_fp = $_POST['desconto_input'];
...
$n_itens = count($_SESSION['compra-interna']);
$evento = (int) $_SESSION['usuario-carnaval'];
$retorno = false;

//Pagamento multiplo
$multiplo = ($forma == 10) ? true : false;
...
if($n_itens > 0) {
	foreach ($_SESSION['compra-interna'] as $key => $carrinho) {
		...
		$_SESSION['compra-interna'][$key]['qtde'] = $carrinho['qtde'] = (int) $quantidade[$key];
...
if($retorno) {
	...
	<script type="text/javascript">
		location.href='<? echo SITE; ?>compras/novo/adicionais/';
	</script>
	<?
	exit();
}
...
// $link_retorno = SITE.'financeiro/detalhes/';
	$link_retorno = SITE.'financeiro/detalhes/';
...
if(is_numeric($cliente) && !empty($forma) && !empty($evento) && ($n_itens > 0)) {
	...
	$vendedor = (int) $_SESSION['us-cod'];

	/*$sql_last = sqlsrv_query($conexao, "SELECT MAX(LO_COD) AS cod FROM loja", $conexao_params, $conexao_options);
	if(sqlsrv_num_rows($sql_last) > 0) {
		$arcompra = sqlsrv_fetch_array($sql_last);
		$compra = $arcompra['cod'] + 1;
	}

	$sql_ins_compra = sqlsrv_query($conexao, "INSERT INTO loja (LO_COD, LO_EVENTO, LO_CLIENTE, LO_PARCEIRO, LO_DATA_COMPRA, LO_DELIVERY, LO_FORMA_PAGAMENTO, LO_CLI_NOME, LO_NUM_ITENS, LO_VENDEDOR, LO_RETIRADA, LO_CLI_PERIODO, LO_COMISSAO, LO_COMISSAO_RETIDA, LO_DEADLINE, LO_CONCIERGE, LO_ORIGEM) VALUES ($compra, $evento, $cliente, $canal, GETDATE(), $delivery, $forma, '$nome_cliente', $n_itens, $vendedor, '$retirada', '$periodo', $comissao, $retida, $deadline, $vendedor_externo, '$origem')", $conexao_params, $conexao_options);*/

	$sql_ins_compra = sqlsrv_query($conexao, "INSERT INTO loja (LO_EVENTO, LO_CLIENTE, LO_PARCEIRO, LO_DATA_COMPRA, LO_DELIVERY, LO_FORMA_PAGAMENTO, LO_CLI_NOME, LO_NUM_ITENS, LO_VENDEDOR, LO_RETIRADA, LO_CLI_PERIODO, LO_COMISSAO, LO_COMISSAO_RETIDA, LO_DEADLINE, LO_CONCIERGE, LO_ORIGEM) VALUES ($evento, $cliente, $canal, GETDATE(), $delivery, $forma, '$nome_cliente', $n_itens, $vendedor, '$retirada', '$periodo', $comissao, $retida, $deadline, $vendedor_externo, '$origem')", $conexao_params, $conexao_options);
	$compra = getLastId();
	...
	$loja_frisas_carrinho = $_SESSION['compra-interna'];
	...
	foreach ($_SESSION['compra-interna'] as $key => $carrinho) {
		...
		for ($iitem=1; $iitem <=$carrinho['qtde'] ; $iitem++) {
			...
			for ($iitemvaga=1; $iitemvaga <= $item_vagas; $iitemvaga++) { 
				...
				//Cadastrar item
				$sql_ins_item = sqlsrv_query($conexao, "INSERT INTO loja_itens (LI_COMPRA, LI_INGRESSO, LI_NOME, LI_ID, LI_VALOR_TABELA, LI_VALOR, LI_EXCLUSIVIDADE, LI_EXCLUSIVIDADE_VAL, LI_DESCONTO, LI_OVER_INTERNO, LI_OVER_EXTERNO) VALUES ($compra, ".$carrinho['item'].", '$nome_cliente', $item_id, '$item_valor_tabela', '".$carrinho['valor']."', '$excl', '".$exclusividadeval[$key]."', '$item_desconto', '$item_overinterno', '$item_overexterno')", $conexao_params, $conexao_options);
				$item = getLastId();
	...
	if($delivery) {
		...
		foreach ($_SESSION['compra-interna'] as $delk => $del) { array_push($del_ar, $del['item']); }
	...
	if ($novo_desconto_fp > 0) {
		$valor = $valor - $novo_desconto_fp;
	}
	...
	if($multiplo) { 
		$link_retorno_multiplo = 'https://ingressos.foliatropical.com.br/controle2014/compras/pagamento-multiplo/'.$compra;
		echo "<script>
		alert('Compra confirmada');
		location.href='".$link_retorno_multiplo."';
		</script>";
	}
