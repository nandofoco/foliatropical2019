...
$sql_ingressos = sqlsrv_query($conexao, "DECLARE @ingressos TABLE (ingressos_compra_valor FLOAT, ingressos_compra INT, ingressos_compra_estoque INT, ingressos_venda INT);
	...
	SELECT 
	ISNULL((SELECT SUM(VE_VALOR) FROM vendas WHERE VE_EVENTO='$evento' AND ".$filtros['tipos'][$tipo]." AND VE_BLOCK='0' AND D_E_L_E_T_='0'), 0) as ingressos_compra_valor, 
	ISNULL((SELECT COUNT(CO_COD) FROM compras WHERE CO_EVENTO='$evento' AND ".$filtros['compras']['tipos'][$tipo]." AND CO_BLOCK='0' AND D_E_L_E_T_='0' AND CO_ESTOQUE IS NULL), 0) as ingressos_compra, 
	ISNULL((SELECT SUM(CO_ESTOQUE) FROM compras WHERE CO_EVENTO='$evento' AND ".$filtros['compras']['tipos'][$tipo]." AND CO_BLOCK='0' AND D_E_L_E_T_='0' AND CO_ESTOQUE IS NOT NULL), 0) as ingressos_compra_estoque, 
	ISNULL((SELECT SUM(VE_ESTOQUE) FROM vendas WHERE VE_EVENTO='$evento' AND ".$filtros['tipos'][$tipo]." AND VE_BLOCK='0' AND D_E_L_E_T_='0'), 0) as ingressos_venda;
...
$query_ingressos_dias = "";

// Busca pelos valores por tipo
foreach ($filtros['dias'] as $d => $dias) { $query_ingressos_dias .= " SUM(CASE WHEN $dias THEN VE_ESTOQUE ELSE 0 END) AS total_dia_".$d.", "; }
...
$sql_ingressos = sqlsrv_query($conexao, "SELECT 
	...
	FROM vendas 
	WHERE VE_EVENTO='$evento' AND ".$filtros['tipos'][$tipo]." AND D_E_L_E_T_='0'", $conexao_params, $conexao_options);

if(sqlsrv_num_rows($sql_ingressos)) $ar_ingressos = sqlsrv_fetch_array($sql_ingressos);
...
<section id="conteudo">
		...
		<div class="painel-colunas">
			<div class="colunas duas-colunas">
				...
				foreach ($conf['status'] as $status => $r) {
					...
					if($status_link) {
						$status_tag = 'a';
						$status_link = 'href="'.$item_link.'?tipo='.$tipo.'&a='.$status.'" ';
					} else {
						$status_tag = 'div';
					}
			...
			<div class="colunas uma-coluna">
				<a href="<? echo SITE; ?>relatorios-lista-produtos-dias.php?tipo=<? echo $tipo; ?>" class="fancybox fancybox.iframe width1480">Movimentação Diária<br/>Por produtos/Todos os dias</a>
				<table>	
					<thead>
						<tr>
							<th>Data</th>
					...
					<tbody>
						<tr id="tempo-real" data-tipo="<? echo $tipos; ?>">
	...
	<section class="secao graficos">
		...
		foreach ($conf['dias'] as $dia => $r) {
			...
			$dia_titulo = $r['titulo'];
			$dia_link = $r['link'];
			$dia_cortesia = (bool) $conf['tipos'][$tipo]['cortesia'];
			$dia_exibe_valor = (bool) $r['valor'];
			...
			// $dia_total = $ar_relatorio['valor_'.$tipo.'_'.$dia];
			$dia_total = $ar_ingressos['total_dia_'.$dia];

		?>
		<div class="coluna<? if($idias > 3) { echo ' bottom'; } if(($idias%3) == 0) { echo ' last'; } ?>">
			<div class="box <? echo $tipo; ?>">
				<h2><? echo $dia_titulo; ?> DIA</h2>
				<div class="grafico vertical">
					<?

					$porcentagem_grafico = 0;

					$qtde_grafico = $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_pagos'];
					$porcentagem_grafico = ($qtde_grafico > 0) ? (($qtde_grafico*100) / $dia_total) : 0;
				...
				<div class="float">
					...
					$status_qtde = $ar_ingressos['total_dia_'.$dia] - $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_saida'] - $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_cortesias'] - $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_permutas'] - $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_promoter'];

					// echo $ar_ingressos['total_dia_'.$dia].'<br/>';
					// echo $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_saida'].'<br/>';
					// echo $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_cortesias'].'<br/>';
					// echo $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_permutas'].'<br/>';
					// echo $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_promoter'].'<br/>';

					$status_porcentagem = ($status_qtde > 0) ? (($status_qtde*100) / $dia_total) : 0;
					...
					foreach ($conf['status'] as $status => $r) {
						...
						$status_valor = $ar_relatorio['valor_'.$tipo.'_'.$dia.'_'.$status];
						$status_qtde = $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_'.$status];

						// $status_porcentagem = ($status_valor > 0) ? (($status_valor*100) / $dia_total) : 0;
						$status_porcentagem = ($status_qtde > 0) ? (($status_qtde*100) / $ar_ingressos['total_dia_'.$dia]) : 0;

						$status_exibe = true;

						
						// Para os ingressos de Folia e Superfolia a Cortesia nao é somada a Permuta, nos demais sim
						if(!$dia_cortesia && $status == 'cortesias') $status_exibe = false; 
						if(!$dia_cortesia && $status == 'permutas') {
							$status_valor += $ar_relatorio['valor_'.$tipo.'_'.$dia.'_cortesias'];
							$status_qtde += $ar_relatorio['qtde_'.$tipo.'_'.$dia.'_cortesias'];
						} 
					...
					?>					
					<a href="<? echo SITE; ?>relatorios/detalhes/<? echo $tipo; ?>/dia/<? echo $dia; ?>/">ver detalhes</a>
				</div>
				<div class="clear"></div>
			</div>
			<div class="clear"></div>
	...
	<footer class="controle">
		<a href="<? echo SITE; ?>relatorios/" class="cancel coluna">Voltar</a>
		<div class="clear"></div>
	</footer>
