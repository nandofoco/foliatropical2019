...
$comissao = format($_POST['comissao']);
...
//Email já cadastrado
$sql_exist = sqlsrv_query($conexao_sankhya, "SELECT EMAIL FROM TGFPAR WHERE EMAIL='$email' AND VENDEDOR='S' $search_cod", $conexao_params, $conexao_options);
...
//CPF já cadastrado 
/*$sql_exist_cpf = sqlsrv_query($conexao_sankhya, "SELECT CGC_CPF FROM TGFPAR WHERE CGC_CPF='$cpfcnpj' AND VENDEDOR='S' $search_cod", $conexao_params, $conexao_options);
...
if(!empty($nome)) {
	...
	$cupom = $cupoml = empty($cupom) ? 'FOLIA'.strtoupper(substr(str_replace('-', '', toAscii($nome)), 0, 10)) : 'FOLIA'.$cupom;

	//Valor Unico
	$sql_nome_unico = sqlsrv_query($conexao_sankhya, "SELECT CUPOM FROM TGFPAR WHERE CUPOM='$cupom' $search_cod", $conexao_params, $conexao_options);
	...
	if(sqlsrv_num_rows($sql_nome_unico) > 0) {
		...
		while($existe_cupom == true){
			$sql_cupom = sqlsrv_query($conexao_sankhya, "SELECT CUPOM FROM TGFPAR WHERE CUPOM='$cupom' $search_cod", $conexao_params, $conexao_options);
...
// if(!$erro && !$editar && !empty($nome) && !empty($email) && !empty($cpfcnpj) && !empty($telefone) && !empty($cep) && !empty($endereco) && !empty($numero) && !empty($bairro) && !empty($cidade) && !empty($estado) && !empty($comissao)) {
if(!$erro && !$editar && !empty($nome) && !empty($email) && !empty($telefone) && !empty($cep) && !empty($endereco) && !empty($numero) && !empty($bairro) && !empty($cidade) && !empty($estado)) {
	...
	$md5_codigo_aleatorio = $senha = md5($codigo_aleatorio);
	...
	if($n_end == 0) {
		...
		$sql_insert_endereco = sqlsrv_query($conexao_sankhya, "INSERT INTO TSIEND (CODEND, NOMEEND, DTALTER) VALUES ((SELECT ISNULL(MAX(CODEND),0) + 1 FROM TSIEND), '$endereco', GETDATE())", $conexao_params, $conexao_options);
			/*if($sql_insert_endereco !== false)*/ $getlastid = sqlsrv_fetch_array(sqlsrv_query($conexao_sankhya, "SELECT ISNULL(MAX(CODEND),0) AS id FROM TSIEND", $conexao_params, $conexao_options));
			$codend = $getlastid['id'];
	} else {
	...
	if($n_bai == 0) {
		...
		/*if($sql_insert_bairro !== false)*/ $getlastid = sqlsrv_fetch_array(sqlsrv_query($conexao_sankhya, "SELECT ISNULL(MAX(CODBAI),0) AS id FROM TSIBAI", $conexao_params, $conexao_options));
		$codbai = $getlastid['id'];
	} else {
	...
	if($n_cid == 0) {
		...
		/*if($sql_insert_cidade !== false)*/ $getlastid = sqlsrv_fetch_array(sqlsrv_query($conexao_sankhya, "SELECT ISNULL(MAX(CODCID),0) AS id FROM TSICID", $conexao_params, $conexao_options));
		$codcid = $getlastid['id'];
	} else {
	...
	$sql_insert = sqlsrv_query($conexao_sankhya, "INSERT INTO TGFPAR (CODPARC, NOMEPARC, RAZAOSOCIAL, IDENTINSCESTAD, EMAIL, TELEFONE, CGC_CPF, TIPPESSOA, CEP, CODEND, NUMEND, COMPLEMENTO, CODBAI, CODCID, CLIENTE, VENDEDOR, CODBCO, CODAGE, CODCTABCO, DTCAD, DTALTER, BLOQUEAR, AD_COMISSAO, AD_SENHA, TIPO, CUPOM, DESCONTO) VALUES ((SELECT ISNULL(MAX(CODPARC),0) + 1 FROM TGFPAR),'$nome','$razaosocial','$inscricao','$email','$telefone','$cpfcnpj','$pessoa','$cep','$codend','$numero','$complemento','$codbai','$codcid','N','S','$banco','$agencia','$conta',GETDATE(),GETDATE(),'N', '$comissao', '$senha', '$tipo', '$cupom', '$desconto')", $conexao_params, $conexao_options);
	...
	if(is_numeric($cod)) {
		...
		$mail->Subject = "Folia Tropical - Cadastro de Parceiro";  //ASSUNTO DA MENSAGEM
		...
		$assunto = utf8_decode("Folia Tropical - Cadastro de Parceiro");
		$mensagem = utf8_decode($msg);

		//enviarEmail($remetente_nome, $remetente_email, $destinatario_nome, $destinatario_email, $assunto, $mensagem);

		$resposta = "Parceiro cadastrado com sucesso.";

		//-----------------------------------------------------------------------------//

		// Inserir o usuario
		$sql_insert = sqlsrv_query($conexao, "INSERT INTO usuarios (US_NOME, US_EMAIL, US_SENHA, US_LOGIN, US_GRUPO, US_FILIAL, US_PARCEIRO) VALUES ('$nome','$email','$md5_codigo_aleatorio','$email','VIN',1,'$cod')", $conexao_params, $conexao_options);
		$codus = getLastId();

		//gravar permissoes
		$menus_cod = '1,4,9';
		$submenus_cod = '1,13,14';
		$sql_permissoes = sqlsrv_query($conexao, "INSERT INTO menu_permissoes (MP_USUARIO, MP_MENU, MP_SUBMENU) VALUES ('$codus','$menus_cod','$submenus_cod')", $conexao_params, $conexao_options);
	...
	<script type="text/javascript">
		alert('<? echo $resposta; ?>');
		location.href='<? echo SITE; ?>parceiros/';
	</script>
	<?
	exit();

// } elseif(!$erro && $editar && is_numeric($cod) && !empty($email) && !empty($cpfcnpj) && !empty($telefone) && !empty($cep) && !empty($endereco) && !empty($numero) && !empty($bairro) && !empty($cidade) && !empty($estado) && !empty($comissao)) {
} elseif(!$erro && $editar && is_numeric($cod) && !empty($email) && !empty($telefone) && !empty($cep) && !empty($endereco) && !empty($numero) && !empty($bairro) && !empty($cidade) && !empty($estado)) {

	if(!empty($senha)) {
		$senha = md5($senha);
		$altera_senha = ", AD_SENHA='$senha'";
		$altera_ussenha = ", US_SENHA='$senha'";
	}
	...
	$sql_update = sqlsrv_query($conexao_sankhya, "UPDATE TOP (1) TGFPAR SET NOMEPARC='$nome', RAZAOSOCIAL='$razaosocial', IDENTINSCESTAD='$inscricao', EMAIL='$email', TELEFONE='$telefone', CGC_CPF='$cpfcnpj', TIPPESSOA='$pessoa', CEP='$cep', CODEND='$codend', NUMEND='$numero', COMPLEMENTO='$complemento', CODBAI='$codbai', CODCID='$codcid', CODBCO='$banco', CODAGE='$agencia', CODCTABCO='$conta', DTALTER=GETDATE(), AD_COMISSAO='$comissao', TIPO='$tipo', CUPOM='$cupom', DESCONTO='$desconto' $altera_senha WHERE CODPARC='$cod'", $conexao_params, $conexao_options);

	// Inserir o usuario
	$sql_insert = sqlsrv_query($conexao, "UPDATE TOP (1) usuarios SET US_NOME='$nome', US_EMAIL='$email' $altera_ussenha , US_LOGIN='$email', US_GRUPO='VIN', US_FILIAL=1 WHERE US_PARCEIRO='$cod'", $conexao_params, $conexao_options);
	...
	<script type="text/javascript">
		alert('<? echo $resposta; ?>');
		location.href='<? echo SITE; ?>parceiros/editar/<? echo $cod; ?>/';
	</script>
	<?
