...
<section id="conteudo" class="relatorio-lista-voucher wide">
	...
	<section class="secao bottom">
		...
		<section class="secao graficos por-dia">
			...
			?>
		<a href="https://ingressos.foliatropical.com.br/controle2014/gera-relatorio-md.php?data=<?=$_GET['data'];?>" class="btn-exportar">Exportar para o Excel</a>
		...
		<table class="lista mini tablesorter-nopager">
			...
			if(count($rows) > 0) {
				...
				foreach ($rows as $line) {
					...
					$item_vaga = utf8_encode($line['VAGAS']);
					$item_tipo_especifico = utf8_encode($line['ESPECIFICO']);
					...
					if($item_fechado) { 
						...
						$loja_valor_total_venda = $loja_valor_total_venda/$item_vaga;
						$loja_lucro = $loja_lucro/$item_vaga;
					} 

					$loja_valor_total_venda = number_format($loja_valor_total_venda, 2, ",", ".");
