	...
	<section class="padding">
		...
		<section id="financeiro-cupom-camisas">
			...
			<section id="cupom-pagamento" class="financeiro">
				<? if ($cupom_cod > 0){ ?>					
					<span class="cupom">
						<? echo $cupom_nome; ?> •  <? echo $cupom_codigo; ?> • Desconto de  <? echo ($cupom_tipo == 1) ? round($cupom_valor)."%" : 'R$ '.number_format($cupom_valor, 2, ',', '.'); ?>
		...
		<footer class="controle">
			...
			<? if (!$loja_cancelado && $administrador){ ?>
				...
				<? $link_cancelar = ($loja_pago && $cartao_credito && ($loja_status_transacao == 6)) ? 'cancelar' : 'excluir'; ?>

				<a href="<? echo SITE; ?>compras/<? echo $link_cancelar; ?>/<? echo $loja_cod; ?>/" class="button cancelar-compra coluna big confirm" title="Deseja realmente cancelar a compra?">Cancelar compra</a>
			<? } ?>
