...
<section id="conteudo">
	<header class="titulo">
		<h1>Agendamentos <span>Pendentes</span></h1>
		<form id="busca-lista" class="busca-lista" method="get" action="<? echo SITE; ?>agendamentos/pendentes/">
			<p class="coluna">
				<label for="busca-lista-input" class="infield">Pesquisar</label>
				<? if(!empty($q)){ ?><a href="<? echo SITE; ?>agendamentos/pendentes/" class="limpar-busca">&times;</a><? } ?>
	...
	<section class="secao bottom">
		<table class="lista tablesorter-nopager">
			...
			?>
				<tr>
					<td colspan="6" class="nenhum">Nenhum agendamento pendente.</td>
				</tr>
			<?
			}
			?>
        ...
        <div class="pager-tablesorter">
	        <a href="<? echo SITE; ?>agendamentos/pendentes/<? if(!empty($q)) echo '?q='.urlencode(utf8_encode($q)); ?>" class="first"></a>
	        <a href="<? echo SITE; ?>agendamentos/pendentes/?<? if(!empty($q)) echo 'q='.urlencode(utf8_encode($q)).'&'; ?>p=<? echo ($p > 1) ? ($p - 1) : 1; ?>" class="prev"></a>
	        <span class="pagedisplay"><? echo $p; ?>/<? echo $total_paginas; ?></span>
	        <a href="<? echo SITE; ?>agendamentos/pendentes/?<? if(!empty($q)) echo 'q='.urlencode(utf8_encode($q)).'&'; ?>p=<? echo ($p < $total_paginas) ? ($p + 1) : $total_paginas; ?>" class="next"></a>
	        <a href="<? echo SITE; ?>agendamentos/pendentes/?<? if(!empty($q)) echo 'q='.urlencode(utf8_encode($q)).'&'; ?>p=<? echo $total_paginas; ?>" class="last"></a>
