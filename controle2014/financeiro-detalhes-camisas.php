...
if(!empty($cod) && !empty($evento)) {
	...
	$sql_loja = sqlsrv_query($conexao, "SELECT TOP 1 l.*, (CONVERT(VARCHAR, l.LO_DATA_COMPRA, 103)+' '+SUBSTRING(CONVERT(VARCHAR, l.LO_DATA_COMPRA, 108),1,5)) AS DATA, ISNULL(DATEDIFF (DAY, LO_DATA_PAGAMENTO, GETDATE()), 6) AS DIFERENCA FROM loja l WHERE l.LO_EVENTO='$evento' AND l.LO_BLOCK='0' AND l.D_E_L_E_T_='0' AND l.LO_COD='$cod'", $conexao_params, $conexao_options);
	...
	if(sqlsrv_num_rows($sql_loja) > 0) {
		...
		//$sql_itens = sqlsrv_query($conexao, "SELECT LI_COD FROM loja_itens WHERE LI_COMPRA='$loja_cod' AND LI_INGRESSO<>'$folia_item' AND D_E_L_E_T_='0'", $conexao_params, $conexao_options);
		...
		$sql_camisas = sqlsrv_query($conexao, "
			...
			SUM(CASE WHEN CA_TAMANHO='PP' THEN 1 ELSE 0 END) AS PP,
		...
		if(sqlsrv_num_rows($sql_camisas) > 0){
			...
			$camisas_total_tamanho['PP'] = $camisas['PP'];
		...
		$tamanhos_camisas = array('PP','P','M','G','GG','EXG');
...
<section id="conteudo" class="camisas">
	...
	<header class="titulo">
		...
		<h1>Tamanho das <span>Camisas</span></h1>
		<div class="tamanhos">
	...
	<section id="camisas-lista">
		<form id="camisas" method="post" action="<? echo SITE; ?>financeiro/detalhes/camisas/post/">
			...
			if(count($_SESSION['compras-camisas'][$cod]) > 0) {
				...
				foreach ($_SESSION['compras-camisas'][$cod] as $key => $compra) {		
					...
					<a href="'.SITE.'include/camisas-adicionar.php?c='.$cod.'&i='.$key.'&a=excluir" class="remover">&times;</a>
