...
// if(empty($_POST['endereco'])||empty($_POST['cartaoNumero'])||empty($_POST['cpfcnpj'])||empty($_POST['anoValidade'])||empty($_POST['mesValidade'])||empty($_POST['cartaoCodigoSeguranca'])||empty($_POST['formaPagamento'])||empty($_POST['nomeTitular'])){
...
// else if(!validaCPF($cpfcnpj)&&!validaCNPJ($cpfcnpj)){
//     $_SESSION['ALERT-PAGAMENTO-CARTAO'] = array('erro','CPF/CNPJ Inválido');
//     header("Location: ".$_SERVER['HTTP_REFERER']."");
//     exit();
// }
...
if(sqlsrv_num_rows($sql_loja) > 0) {
    ...
    //Autorizar transação autenticada e não-autenticada
    $Pedido->autorizar = 2;
    $Pedido->dadosPortadorNumero = $_POST["cartaoNumero"];
    $Pedido->bin = substr($_POST["cartaoNumero"], 0,6);
    $Pedido->token = 'false';
    
    $Pedido->dadosPortadorVal = $_POST["anoValidade"].str_pad($_POST["mesValidade"], 2, "0", STR_PAD_LEFT);
    $Pedido->dadosPortadorNome = $_POST["nomeTitular"];
    ...
    $Pedido->urlRetorno = ReturnURL($cod);
    $Pedido->clienteEndereco = empty($endereco['CE_ENDERECO'])?"NULL":$endereco['CE_ENDERECO'];
    $Pedido->clienteComplemento = empty($endereco['CE_COMPLEMENTO'])?"NULL":$endereco['CE_COMPLEMENTO'];
    $Pedido->clienteNumero = empty($endereco['CE_NUMERO'])?"NULL":$endereco['CE_NUMERO'];
    $Pedido->clienteBairro = empty($endereco['CE_BAIRRO'])?"NULL":$endereco['CE_BAIRRO'];
    $Pedido->clienteCep = empty($endereco['CE_CEP'])?"NULL":$endereco['CE_CEP'];
    // ENVIA REQUISIÇÃO SITE CIELO
    // if($_POST["tentarAutenticar"] == "sim") // TRANSAÇÃO
    if(true) {
        $objResposta = $Pedido->RequisicaoTransacaoAvs(true);
    ...
    // echo "UPDATE loja SET 
    //     $update_pagamento
        
    //     LO_CLI_COMPLEMENTO = '$complemento',
    //     LO_CLI_NUMERO = '$numero',
    //     LO_CLI_ENDERECO = '$endereco',
    //     LO_CLI_BAIRRO = '$bairro',
    //     LO_CLI_CIDADE = '$cidade',
    //     LO_CLI_ESTADO = '$estado',
    //     LO_CLI_CEP = '$cep',
    ...
    ?>
    XML Retorno:</br>
    <textarea name="" id="" cols="30" rows="10"><?php echo $objResposta->asXML(); ?></textarea><?
    
    exit();

    $link_retorno = $fromcliente ? str_replace('controle2014/', '', SITE).$_POST['lang'].'ingressos/pagamento/cielo/'.$loja_cod : SITE.'compra/retorno/'.$loja_cod;
    ...
    /*?>
        <script type="text/javascript">
            alert('Um erro ocorreu. Verifique todas as informações e tente outra vez!');
            // swal("Erro", "Um erro ocorreu. Verifique todas as informações e tente outra vez!", "error");
            window.location.href = "<?php echo $link_retorno; ?>";
        </script>
    <?php */
    }else{
    ?>
