...
if(!empty($cod)) {
	...
	if(sqlsrv_num_rows($sql_co) > 0) {
		...
		$co_status_anterior = $co['LO_STATUS_TRANSACAO'];
		...
		if($co_status == '6'){
			...
			if($co_status_anterior != $co_status) {
				...
				if(sqlsrv_num_rows($sql_cliente) > 0) {
					...
					//Envio de SMS
					$sms = "O pagamento do pedido n. $ foi confirmado. Central de Atendimento Folia Tropical: 21 3202 6000";
			...
			$sql_cp = sqlsrv_query($conexao, "UPDATE TOP (1) loja SET LO_PAGO=0 WHERE LO_COD=$cod", $conexao_params, $conexao_options);
		}
		...
		$link = 'financeiro/detalhes/'.$cod.'/';
		$resposta = '';

		switch($co_status) {
			case 0:
				$resposta = "O pagamento do pedido ainda não foi feito. Será necessário realizar o pagamento novamente caso já tenha feito!";
				break;	
			case 1:			
				$resposta = "O pagamento do pedido está em andamento!";
				break;
			case 2:
				$resposta = "O pagamento do pedido foi autenticado!";
				break;
			case 3:
				$resposta = "O pagamento do pedido não foi autenticado!";
				break;
			case 5:
			...
			break;
			case 10:			
				$resposta = "O pedido está em processo de autenticação.";
			break;
			case 12:			
				$resposta = "O pedido está em processo de cancelamento.";
			break;
		}
