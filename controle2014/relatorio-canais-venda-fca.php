...
DECLARE @relatorio TABLE (PARCEIRO VARCHAR(255), TIPO VARCHAR(50), FOLIA FLOAT, SUPER FLOAT, FRISA FLOAT, ARQUIBANCADA FLOAT, CAMAROTE FLOAT, TOTAL FLOAT);

INSERT INTO @relatorio (PARCEIRO, TIPO, FOLIA, SUPER, FRISA, ARQUIBANCADA, CAMAROTE, TOTAL)
SELECT
...
SUM(CASE WHEN TIPO = 'frisa' THEN LUCRO ELSE 0 END) AS FRISA,
SUM(CASE WHEN TIPO = 'arquibancada' THEN LUCRO ELSE 0 END) AS ARQUIBANCADA,
SUM(CASE WHEN TIPO = 'camarote' THEN LUCRO ELSE 0 END) AS CAMAROTE,
SUM(LUCRO) AS TOTAL
		...
		FROM (
			...
			*,		
			...
			FROM (
				...
				WHERE d.D_E_L_E_T_=0 AND t.D_E_L_E_T_=0 AND s.D_E_L_E_T_=0 AND t.TI_TAG IN ('lounge', 'super', 'frisa', 'arquibancada', 'camarote')
			) S
		) S 
	) S
) S GROUP BY PARCEIRO;
...
FRISA AS 'FRISA',
ARQUIBANCADA AS 'ARQUIBANCADA',
CAMAROTE AS 'CAMAROTE',
...
FROM (
	...
	SELECT * FROM (
		...
		r.FRISA AS 'FRISA',
		r.ARQUIBANCADA AS 'ARQUIBANCADA',
		r.CAMAROTE AS 'CAMAROTE',
	...
	SELECT * FROM (
		...
		SUM(r.FRISA) AS 'FRISA',
		SUM(r.ARQUIBANCADA) AS 'ARQUIBANCADA',
		SUM(r.CAMAROTE) AS 'CAMAROTE',
		...
		<table class="lista">
            <thead>
                <tr>
                    ...
                    <th class="header"><strong>Folia + Super</strong></th>
                    <th class="header"><strong>Frisa</strong></th>
                    <th class="header"><strong>Camarote</strong></th>
                    <th class="header"><strong>Arquibancada</strong></th>
            ...
            <tbody>
                ...
                while(sqlsrv_next_result($sql_relatorio)) {
                    ...
                    while($relatorio = sqlsrv_fetch_array($sql_relatorio)) {
                        ...
                        $frisa = $relatorio['FRISA'];
                        $arquibancada = $relatorio['ARQUIBANCADA'];
                        $camarote = $relatorio['CAMAROTE'];
                    ...
                    ?>
                        <tr>
                            ...
                            <td>R$ <? echo number_format($foliatropical+$superfolia,2,',','.'); ?></td>
                            <td>R$ <? echo number_format($frisa,2,',','.'); ?></td>
                            <td>R$ <? echo number_format($camarote,2,',','.'); ?></td>
                            <td>R$ <? echo number_format($arquibancada,2,',','.'); ?></td>
