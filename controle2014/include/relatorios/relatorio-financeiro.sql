...
DECLARE @pm_pos TABLE (PM_VALOR FLOAT, PM_LOJA INT)
INSERT INTO @pm_pos (PM_VALOR, PM_LOJA)
SELECT PM_VALOR,PM_LOJA FROM [foliatropical2014].[dbo].[loja_pagamento_multiplo] WHERE PM_FORMA=6;

DECLARE @pm_cheque TABLE (PM_VALOR FLOAT, PM_LOJA INT)
INSERT INTO @pm_cheque (PM_VALOR, PM_LOJA)
SELECT PM_VALOR,PM_LOJA FROM [foliatropical2014].[dbo].[loja_pagamento_multiplo] WHERE PM_FORMA=2;

DECLARE @pm_deposito TABLE (PM_VALOR FLOAT, PM_LOJA INT)
INSERT INTO @pm_deposito (PM_VALOR, PM_LOJA)
SELECT PM_VALOR,PM_LOJA FROM [foliatropical2014].[dbo].[loja_pagamento_multiplo] WHERE PM_FORMA=4;

DECLARE @pm_dinheiro TABLE (PM_VALOR FLOAT, PM_LOJA INT)
INSERT INTO @pm_dinheiro (PM_VALOR, PM_LOJA)
SELECT PM_VALOR,PM_LOJA FROM [foliatropical2014].[dbo].[loja_pagamento_multiplo] WHERE PM_FORMA=3;

DECLARE @pm_outros TABLE (PM_VALOR FLOAT, PM_LOJA INT)
INSERT INTO @pm_outros (PM_VALOR, PM_LOJA)
SELECT SUM(PM_VALOR),PM_LOJA FROM [foliatropical2014].[dbo].[loja_pagamento_multiplo] WHERE PM_FORMA NOT IN (6,2,4,3) GROUP BY PM_LOJA;

DECLARE @loja TABLE (LO_COD INT, LO_CARTAO_NOME VARCHAR(255), LO_CARTAO_BANDEIRA VARCHAR(50), LO_CARTAO VARCHAR(50), LO_PARCELAS INT, LO_ORIGEM VARCHAR(50), 
LO_CLIENTE INT, LO_PARCEIRO INT, LO_VENDEDOR INT, LO_DATA_COMPRA DATETIME, LO_VALOR_TOTAL FLOAT, LO_VALOR_PARCIAL FLOAT, LO_VALOR_ITEM FLOAT, LO_VALOR_ITEM_TABELA FLOAT, LO_VALOR_INGRESSOS FLOAT, LO_VALOR_ADICIONAIS FLOAT, LO_VALOR_TRANSFER FLOAT, LO_VALOR_DELIVERY FLOAT, LO_VALOR_DESCONTO FLOAT, LO_VALOR_OVER_INTERNO FLOAT, LO_VALOR_OVER_EXTERNO FLOAT, LO_COMISSAO INT, LO_COMISSAO_RETIDA TINYINT, LO_COMISSAO_PAGA TINYINT, LO_PAGO TINYINT, LO_DESCONTO_FOLIA TINYINT, LO_DESCONTO_FRISA TINYINT, LO_TID VARCHAR(255), LO_FORMA_PAGAMENTO INT, LI_INGRESSO INT, QTDE INT, ITENS INT);


INSERT INTO @loja (LO_COD, LO_CARTAO_NOME, LO_CARTAO_BANDEIRA, LO_CARTAO, LO_PARCELAS, LO_ORIGEM, LO_CLIENTE, LO_PARCEIRO, LO_VENDEDOR, LO_DATA_COMPRA, LO_VALOR_TOTAL, LO_VALOR_PARCIAL, LO_VALOR_ITEM, LO_VALOR_ITEM_TABELA, LO_VALOR_INGRESSOS, LO_VALOR_ADICIONAIS, LO_VALOR_TRANSFER, LO_VALOR_DELIVERY, LO_VALOR_DESCONTO, LO_VALOR_OVER_INTERNO, LO_VALOR_OVER_EXTERNO, LO_COMISSAO, LO_COMISSAO_RETIDA, LO_COMISSAO_PAGA, LO_PAGO, LO_DESCONTO_FOLIA, LO_DESCONTO_FRISA, LO_TID, LO_FORMA_PAGAMENTO, LI_INGRESSO, QTDE, ITENS)
SELECT l.LO_COD, l.LO_CARTAO_NOME, l.LO_CARTAO_BANDEIRA, l.LO_CARTAO, l.LO_PARCELAS, MAX(l.LO_ORIGEM), l.LO_CLIENTE, MAX(l.LO_PARCEIRO), MAX(l.LO_VENDEDOR),MAX(l.LO_DATA_COMPRA), MAX(l.LO_VALOR_TOTAL), MAX(l.LO_VALOR_PARCIAL), li.LI_VALOR, li.LI_VALOR_TABELA /*(li.LI_VALOR + li.LI_DESCONTO - li.LI_OVER_INTERNO - li.LI_OVER_EXTERNO)*/, MAX(l.LO_VALOR_INGRESSOS), li.LI_VALOR_ADICIONAIS, li.LI_VALOR_TRANSFER, MAX(l.LO_VALOR_DELIVERY), li.LI_DESCONTO, li.LI_OVER_INTERNO, li.LI_OVER_EXTERNO, MAX(l.LO_COMISSAO), MAX(l.LO_COMISSAO_RETIDA), MAX(l.LO_COMISSAO_PAGA), MAX(l.LO_PAGO), MAX(l.LO_DESCONTO_FOLIA), MAX(l.LO_DESCONTO_FRISA), MAX(l.LO_TID), MAX(l.LO_FORMA_PAGAMENTO), li.LI_INGRESSO, COUNT (li.LI_COD), MAX(l.LO_NUM_ITENS)
...
GROUP BY li.LI_INGRESSO, l.LO_COD, l.LO_CARTAO_NOME, l.LO_CARTAO_BANDEIRA, l.LO_CARTAO, l.LO_PARCELAS, l.LO_CLIENTE, li.LI_VALOR, li.LI_VALOR_TABELA, li.LI_VALOR_TRANSFER, li.LI_VALOR_ADICIONAIS, li.LI_DESCONTO, li.LI_OVER_INTERNO, li.LI_OVER_EXTERNO;
	...
	FROM (
		...
		*,		
		...
		FROM (
			...
			pp.PM_VALOR as CARTAO_POS,
			pc.PM_VALOR as CHEQUE,
			pd.PM_VALOR as DEPOSITO,
			pdi.PM_VALOR as DINHEIRO,
			po.PM_VALOR as OUTROS,
			l.LO_TID AS TID,
			...
			LEFT JOIN @pm_pos pp ON pp.PM_LOJA = l.LO_COD
			LEFT JOIN @pm_cheque pc ON pc.PM_LOJA = l.LO_COD
			LEFT JOIN @pm_deposito pd ON pd.PM_LOJA = l.LO_COD
			LEFT JOIN @pm_dinheiro pdi ON pdi.PM_LOJA = l.LO_COD
			LEFT JOIN @pm_outros po ON po.PM_LOJA = l.LO_COD
			...
			LEFT JOIN [foliatropical2014].[dbo].[loja_pagamento_multiplo] pm ON pm.PM_LOJA=l.LO_COD
