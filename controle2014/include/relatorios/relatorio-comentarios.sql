...
FROM (
	...
	*,
	...
	FROM (
		...
		CASE WHEN lia.LIA_COMPRA IS NOT NULL THEN 'SIM' ELSE '' END AS PNE,
		d.ED_DATA
		...
		LEFT JOIN [foliatropical2014].[dbo].[loja_itens_adicionais] lia ON lia.LIA_COMPRA=l.LO_COD AND lia.LIA_ADICIONAL IN (SELECT VA_COD FROM [foliatropical2014].[dbo].[vendas_adicionais] WHERE (VA_NOME_EXIBICAO='pne') AND VA_BLOCK=0 AND D_E_L_E_T_=0)
