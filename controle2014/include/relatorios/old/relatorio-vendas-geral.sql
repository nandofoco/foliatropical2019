...
DECLARE @loja TABLE (LO_COD INT, LO_CLIENTE INT, LO_DATA_COMPRA DATETIME, LO_VALOR_TOTAL FLOAT, LO_VALOR_PARCIAL FLOAT, LO_VALOR_ITEM FLOAT, LO_VALOR_ITEM_TABELA FLOAT, LO_PAGO TINYINT, LO_TID VARCHAR(255), LI_INGRESSO INT, QTDE INT);

INSERT INTO @loja (LO_COD, LO_CLIENTE, LO_DATA_COMPRA, LO_VALOR_TOTAL, LO_VALOR_PARCIAL, LO_VALOR_ITEM, LO_VALOR_ITEM_TABELA, LO_PAGO, LO_TID, LI_INGRESSO, QTDE)
SELECT l.LO_COD, l.LO_CLIENTE, MAX(l.LO_DATA_COMPRA), MAX(l.LO_VALOR_TOTAL), MAX(l.LO_VALOR_PARCIAL), SUM(li.LI_VALOR), SUM(li.LI_VALOR_TABELA), MAX(l.LO_PAGO), MAX(l.LO_TID), li.LI_INGRESSO, COUNT (li.LI_COD)
...
SELECT VOUCHER, CLIENTE, EMAIL, CPF_CNPJ, DATA_COMPRA, TOTAL, VALOR_ITEM, VALOR_ITEM_TABELA, DESCONTO, QTDE, TID, TIPO, ESPECIFICO, DATA, SETOR, FILA, VAGAS, PAGO FROM (
	SELECT 
	l.LO_COD AS VOUCHER,
	...
	CASE WHEN v.VE_TIPO_ESPECIFICO='fechado' AND v.VE_VAGAS>0 THEN l.LO_VALOR_ITEM/l.QTDE ELSE l.LO_VALOR_ITEM END AS VALOR_ITEM,
	CASE WHEN v.VE_TIPO_ESPECIFICO='fechado' AND v.VE_VAGAS>0 THEN l.LO_VALOR_ITEM_TABELA/l.QTDE ELSE l.LO_VALOR_ITEM_TABELA END AS VALOR_ITEM_TABELA,
