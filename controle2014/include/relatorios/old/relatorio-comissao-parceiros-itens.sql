...
DECLARE @loja_itens TABLE (LI_COMPRA INT, QTDE INT, LI_VALOR_TABELA FLOAT, LI_VALOR FLOAT, VE_TIPO INT, VE_ESTOQUE INT, VE_SETOR INT, VE_DIA INT, VE_FILA VARCHAR(255), VE_VAGAS INT, VE_TIPO_ESPECIFICO VARCHAR(255));
INSERT INTO @loja_itens (LI_COMPRA, QTDE, LI_VALOR_TABELA, LI_VALOR, VE_TIPO, VE_ESTOQUE, VE_SETOR, VE_DIA, VE_FILA, VE_VAGAS, VE_TIPO_ESPECIFICO)
SELECT 
...
SELECT * FROM (
	...
	ISNULL(li.VE_TIPO_ESPECIFICO,'') AS ESPECIFICO,
	--REPLACE(d.ED_NOME,'°','o') AS DIA,
	CONVERT(VARCHAR, d.ED_DATA, 103) AS DATA,
	s.ES_NOME AS SETOR,
	ISNULL(li.VE_FILA,'') AS FILA,
	ISNULL(li.VE_VAGAS,'') AS VAGAS,
	...
	(CASE WHEN li.LI_VALOR_TABELA<li.LI_VALOR THEN li.LI_VALOR_TABELA ELSE li.LI_VALOR END * (c.AD_COMISSAO / 100)) AS VALOR_COMISSAO,
	...
	LEFT JOIN @loja_itens li ON li.LI_COMPRA = l.LO_COD
	LEFT JOIN [foliatropical].[dbo].[tipos] t ON t.TI_COD=li.VE_TIPO
	LEFT JOIN [foliatropical].[dbo].[eventos_dias] d ON d.ED_COD=li.VE_DIA
	LEFT JOIN [foliatropical].[dbo].[eventos_setores] s ON s.ES_COD=li.VE_SETOR
