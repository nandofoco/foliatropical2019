...
DECLARE @loja TABLE (LO_COD INT, LO_CLIENTE INT, LO_PARCEIRO INT, LO_DATA_COMPRA DATETIME, LO_DATA_ENTREGA DATETIME, LO_VALOR_TOTAL FLOAT, LO_PAGO TINYINT, LO_TID VARCHAR(255));

INSERT INTO @loja (LO_COD, LO_CLIENTE, LO_PARCEIRO, LO_DATA_COMPRA, LO_DATA_ENTREGA, LO_VALOR_TOTAL, LO_PAGO, LO_TID)
SELECT LO_COD, LO_CLIENTE, LO_PARCEIRO, LO_DATA_COMPRA, LO_DATA_ENTREGA, LO_VALOR_TOTAL, LO_PAGO, LO_TID
FROM [foliatropical].[dbo].[loja] WHERE LO_EVENTO=1 AND LO_ENVIADO=1 AND D_E_L_E_T_=0;
...
SELECT VOUCHER, PARCEIRO, CLIENTE, EMAIL, CPF_CNPJ, DATA_COMPRA, DATA_ENTREGA, TID, PAGO FROM (
	SELECT 
	l.LO_COD AS VOUCHER,
	...
	(CONVERT(VARCHAR, l.LO_DATA_ENTREGA, 103)+' '+SUBSTRING(CONVERT(VARCHAR, l.LO_DATA_ENTREGA, 108),1,5)) AS DATA_ENTREGA,
