...
WHERE LIA_ADICIONAL IN (SELECT VA_COD FROM [foliatropical].[dbo].[vendas_adicionais] WHERE VA_NOME_INSERCAO='transfer' OR VA_NOME_INSERCAO='transferinout')
...
FROM [foliatropical].[dbo].[transportes_agendamento] ta
LEFT JOIN [foliatropical].[dbo].[loja_itens] li ON li.LI_COD=ta.TA_ITEM
LEFT JOIN [foliatropical].[dbo].[loja] l ON l.LO_COD=li.LI_COMPRA
LEFT JOIN [foliatropical].[dbo].[transportes_horarios] th ON th.TH_COD=ta.TA_HORARIO
LEFT JOIN [foliatropical].[dbo].[transportes] t ON t.TR_COD=th.TH_TRANSPORTE
LEFT JOIN [foliatropical].[dbo].[roteiros] r ON r.RO_COD=t.TR_ROTEIRO
...
LEFT JOIN [foliatropical].[dbo].[tipos] tp ON tp.TI_COD=v.VE_TIPO
...
GROUP BY li.LI_COD
ORDER BY ROTEIRO ASC, TRANSPORTE ASC, HORARIO ASC, ITEM ASC;