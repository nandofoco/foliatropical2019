...
DECLARE @loja TABLE (LO_COD INT, LO_CLIENTE INT, LO_DATA_COMPRA DATETIME, LO_VALOR_TOTAL FLOAT, LO_PAGO TINYINT, LO_TID VARCHAR(255), LI_INGRESSO INT, QTDE INT);

INSERT INTO @loja (LO_COD, LO_CLIENTE, LO_DATA_COMPRA, LO_VALOR_TOTAL, LO_PAGO, LO_TID, LI_INGRESSO, QTDE)
SELECT l.LO_COD, l.LO_CLIENTE, MAX(l.LO_DATA_COMPRA), MAX(l.LO_VALOR_TOTAL), MAX(l.LO_PAGO), MAX(l.LO_TID), li.LI_INGRESSO, COUNT (li.LI_COD)
FROM [foliatropical].[dbo].[loja] l, [foliatropical].[dbo].[loja_itens] li WHERE l.LO_EVENTO=1 AND (l.LO_PARCEIRO IS NULL OR l.LO_PARCEIRO=0) AND l.LO_COD=li.LI_COMPRA AND l.D_E_L_E_T_=0 AND li.D_E_L_E_T_=0
...
SELECT VOUCHER, CLIENTE, EMAIL, CPF_CNPJ, DATA_COMPRA, TOTAL, QTDE, TID, TIPO, ESPECIFICO, DATA, SETOR, FILA, VAGAS, PAGO FROM (
	SELECT 
	l.LO_COD AS VOUCHER,
