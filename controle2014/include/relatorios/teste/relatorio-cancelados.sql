...
FROM [foliatropical2014].[dbo].[loja] l, [foliatropical2014].[dbo].[loja_itens] li WHERE l.LO_EVENTO=%evento% AND l.LO_COD=li.LI_COMPRA AND l.D_E_L_E_T_=1 /*vendedor*/
...
SELECT COD, DESCONTO FROM (
	...
	FROM (
		...
		WHERE l.LO_EVENTO=%evento% AND l.LO_PARCEIRO=54 AND l.LO_COD=li.LI_COMPRA AND l.D_E_L_E_T_=1 /*AND d.ED_DATA IN('2015-02-15', '2015-02-16')*/ AND t.TI_TAG='lounge'
		GROUP BY l.LO_COD
	) S
) S WHERE DESCONTO > 0
...
DECLARE @loja_excluidas TABLE (LE_COMPRA INT, LE_USUARIO INT, LE_DATA DATETIME);

INSERT INTO @loja_excluidas (LE_COMPRA, LE_USUARIO, LE_DATA)
SELECT LE_COMPRA, MAX(LE_USUARIO), MAX(LE_DATA) FROM [foliatropical2014].[dbo].[loja_excluidas] GROUP BY LE_COMPRA ORDER BY MAX(LE_DATA) DESC;
...
USUARIO_CANCELAMENTO AS 'USUARIO CANCELAMENTO',
DATA_CANCELAMENTO AS 'DATA DO CANCELAMENTO',
	...
	FROM (
		...
		*,		
		...
		FROM (
			...
			uex.US_NOME AS USUARIO_CANCELAMENTO,
			ex.LE_DATA AS DATA_CANCELAMENTO,
			...
			LEFT JOIN @loja_excluidas ex ON l.LO_COD=ex.LE_COMPRA
			LEFT JOIN [foliatropical2014].[dbo].[usuarios] uex ON ex.LE_USUARIO=uex.US_COD
