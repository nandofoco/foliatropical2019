...
OVER_INTERNO_TOTAL AS 'OVER INTERNO TOTAL',
...
TRANSFER,
DELIVERY,
VALOR_TOTAL_VENDA+TRANSFER AS 'VALOR TOTAL VENDA',
OVER_EXTERNO_TOTAL AS 'OVER EXTERNO TOTAL',
COMISSAO,
VALOR_ITEM AS 'VALOR ITEM',
VALOR_COMISSAO AS 'VALOR COMISSAO',
TAXA_CARTAO AS 'TAXA CARTAO',
(VALOR_TOTAL_VENDA - OVER_EXTERNO_TOTAL - (CASE WHEN TAXA_CARTAO IS NOT NULL THEN (VALOR_TOTAL_CALCULO_COMISSAO - VALOR_COMISSAO) * (TAXA_CARTAO / 100) ELSE 0 END)) AS LUCRO,
...
FROM (
	...
	SELECT
	*,
	(TRANSFER_UNITARIO * CASE WHEN ESPECIFICO='fechado' THEN VAGAS ELSE QTDE END) AS TRANSFER,
	...
	(OVER_INTERNO * CASE WHEN ESPECIFICO='fechado' THEN VAGAS ELSE QTDE END) AS OVER_INTERNO_TOTAL,
	(OVER_EXTERNO * CASE WHEN ESPECIFICO='fechado' THEN VAGAS ELSE QTDE END) AS OVER_EXTERNO_TOTAL,
	((PRECO_AUTORIZADO * QTDE) * COMISSAO / 100) AS VALOR_COMISSAO
	FROM (
		SELECT
		*,		
		(VALOR_TABELA + (CASE WHEN ESPECIFICO='fechado' THEN OVER_INTERNO*VAGAS ELSE OVER_INTERNO END)) AS PRECO_AUTORIZADO,
		(VALOR_TABELA - VALOR_DESCONTO + (CASE WHEN ESPECIFICO='fechado' THEN OVER_INTERNO*VAGAS ELSE OVER_INTERNO END) + OVER_EXTERNO) AS PRECO_PRATICADO,
		CASE 
		...
		FROM (
			...
			(CASE WHEN v.VE_TIPO_ESPECIFICO='fechado' THEN l.QTDE/v.VE_VAGAS ELSE l.QTDE END) AS QTDE,
			l.ITENS AS ITENS,
