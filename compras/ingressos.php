...
if(count($_SESSION['compra-site']) > 0) {
    ...
    <section class="secao" id="carrinho">
		...
		<form id="compras-adicionais" class="padrao" method="post" action="<? echo SITE.$link_lang; ?>ingressos/post/">
			...
			<input type="hidden" name="link_tipo" id="link_tipo" value="<? echo $link_tipo; ?>" />
            ...
            <table class="lista">
                        ...
                        <tr class="big">
                            ...
                            <td class="qtde">
                                ...
                                <select name="quantidade[<? echo $key; ?>]" class="input" rel="<? echo $key; ?>">
                                    <? for($q = 1; $q <= $ingressos_estoque; $q++) { ?>
                                    <option value="<? echo $q; ?>" <? if($q == $carrinho['qtde']) { echo 'selected'; } ?>><? echo $q; ?></option>
                                    <? } ?>
                                </select>

                               <!--  <input type="text" class="mask" id="qtde_ingressos<? echo $key; ?>" name="quantidade[<? echo $key; ?>]" min="1" max="<? echo $ingressos_estoque; ?>" value="<? echo $carrinho['qtde'];?>" onblur="verificaEstoque(<? echo $ingressos_estoque; ?>,<? echo $key; ?>)"> -->
				...
				//Verifica a exestência de desconto para caso sejam comprados 2 ingressos em dias diferentes 
	             ...
	             $_SESSION['desconto_ft_porc'] = $ar_desconto['DES_VALOR'];

	             $ingressos_valor_soma_anterior = $ingressos_valor_soma;

				if (count($_SESSION['data_ingresso_desconto']) >= 2)
				{
					$desconto_folia = $ingressos_valor_soma * $porcentagem_desconto;

					$entrouDescontoFolia = true;
				...
				if($_SESSION['compra-cupom']['usuario'] == $usuario_cod) {
					...
					if($n_cupom > 0) {
						...
						switch ($cupom_tipo) {
							case 1:
								$cupom_valor_desconto = (($cupom_valor * $ingressos_valor_soma_anterior) / 100);
							break;
				...
				if ($entrouDescontoFolia) {
					unset($cupom_cod);
					unset($_SESSION['compra-cupom']['cod']);

					//$sql_rm_desconto = sqlsrv_query($conexao, "UPDATE loja SET LO_DESCONTO=0, LO_VALOR_DESCONTO=0 WHERE LO_COD='' ", $conexao_params, $conexao_options);
				}
				...
				?>
					...
					<tr/>
						...
						<td colspan="3" class="total-final">
							<strong><? echo $lg['total']; ?></strong>
							<span class="valor valor-final">R$ <? echo number_format($loja_valor_total, 2, ",", "."); ?></span>
						</td>
					</tr>
            ...
            <footer class="controle">
				...
				<a href="<? echo SITE.$link_lang.$link_tipo; ?>" class="continuar"><? echo $lg['continuar_comprando']; ?></a>
	...
	<script type="text/javascript">
        ...
        var qtde_ingressos = $('#qtde_ingressos'+id).val();


        if (qtde_ingressos <= 10) {
                if (qtde_ingressos > qtde_estoque) {
                alert('Apenas '+ qtde_estoque + ' ingressos disponíveis no estoque!');
                $('#qtde_ingressos'+id).val(qtde_estoque);
            }
        }
        else {
            alert('Limite de 10 ingressos por pessoa.');
            $('#qtde_ingressos'+id).val(10);
        }
