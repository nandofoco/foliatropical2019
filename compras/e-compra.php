...
if($retorno) {
	...
	?>
	<script type="text/javascript">
		location.href='<? echo SITE.$link_lang.$link_tipo; ?>ingressos/';
	</script>
	<?
	exit();
}
...
$link_retorno = SITE.$link_lang.$link_tipo.'ingressos/';
...
if(is_numeric($cliente) && !empty($forma) && !empty($evento) && ($n_itens > 0)) {
	...
	if($compra > 0) {
		...
		$link_retorno = SITE.$link_lang.$link_tipo.'pagamento/'.$compra.'/';
		...
		// Busca o desconto Folia Tropical da sessão e aplica ao valor total
		if (isset($desconto_ft) && $desconto_ft > 0) {
			$valor = $valor - $desconto_ft;
			$desconto = 0;
		}
		else
		{
		...
		$sql_update_compra = sqlsrv_query($conexao, "UPDATE TOP (1) loja SET 
			...
			LO_DESCONTO='$aplicaDesconto',
			LO_VALOR_DESCONTO='$desconto',
			LO_VALOR_DESCONTO_FT='$desconto_ft',
		...
		<script type="text/javascript">
			...
			alert("<?=$lg['compre_adicionais_extra_reserva_confirmada'];?>");
