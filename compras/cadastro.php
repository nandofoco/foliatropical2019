...
$ddi_celular = trim(format($_POST['ddi_celular']));
$telefone = trim(format($_POST['telefone']));
$ddd_celular = trim(format(((int)$_POST['ddd-cel']))."");
...
//-------------------------------------------------------------------------------//
 ...
 if(!$erro && !$editar && !empty($nome) && !empty($pessoa) && !empty($email) && (($pais=="BR" && !empty($cpfcnpj))||($pais!="BR" && !empty($passaporte))) && !empty($celular) && !empty($data_nascimento) && !empty($senha) && !empty($pais)) {
	...
	if(!empty($cod)) {
		...
		if(sqlsrv_num_rows($sql_last) > 0) {
			...
			$assunto = utf8_decode($lg['email_cadastro']);
...
<script type="text/javascript">
    swal({
        title: "<? echo $lg['cadastro_login_titulo']; ?>",
        text: "<? echo $resposta;  ?>",
        html: true,
