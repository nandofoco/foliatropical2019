		...
		<section class="padding">
			...
			<section id="compra-pagamento">
				...
				<section id="cupom-parceiro">
					<form class="controle" id="form-cupom-parceiro" action="<? echo SITE.$link_lang; ?>ingressos/pagamento/paypal/cupom-parceiro/" method="post">
					...
					<? if(!$loja_desconto) { ?>
						<? if ($cupom_cod > 0){ ?>
							<span class="cupom">
								...
								<? if ($cupom_delete){ ?><a href="<? echo SITE.$link_lang; ?>ingressos/pagamento/paypal/cupom/remover/<? echo $cupom_cod; ?>/<? echo $cod; ?>/" class="excluir confirm" title="Deseja remover o cupom &rdquo;<? echo $cupom_nome; ?>&ldquo;">&times;</a><? } ?>
							</span>
						<? } /*else { ?>
						<form class="controle" id="form-cupom-pagamento" action="<? echo SITE.$link_lang; ?>ingressos/pagamento/paypal/cupom/" method="post">
							<input type="hidden" name="cod" value="<? echo $cod; ?>" />
							<input type="hidden" name="paypal" value="true" />
							<p>
		        ...
		        <!-- <form class="form-padrao controle wps-bn" id="form-compra-pagamento" action="https://www.sandbox.paypal.com/cgi-bin/webscr" method="post"> -->
		        <form class="form-padrao controle wps-bn" id="form-compra-pagamento" action="https://www.paypal.com/cgi-bin/webscr" method="post">
		            ...
		            <input type="hidden" name="business" value="douglas@grupopacifica.com.br" />
		            <input type="hidden" name="return" value="<? echo SITE ?>retorno/paypal/sucesso/" />
		            <input type="hidden" name="cancel" value="<? echo SITE ?>retorno/paypal/cancel/" />
		            <input type="hidden" name="notify_url" value="http://www.foliatropical.com.br/controle2014/paypal/notificacao.php" />
					...
					if(sqlsrv_num_rows($sql_item) > 0) {
						...
						while ($item = sqlsrv_fetch_array($sql_item)) {
							...
							//for($q=1; $q<=$item_qtde; $q++){ ?>
							
							<!-- <input type="hidden" name="item_name_<? echo $i; ?>" value="<? echo $produto_descricao; ?>" />
							<input type="hidden" name="amount_<? echo $i; ?>" value="<? echo (float) $produto_valor_unitario / 100; ?>" /> -->
					...
					if($desconto) {
						...
						<!-- <input type="hidden" name="item_name_<? echo $i; ?>" value="desconto" />
						<input type="hidden" name="amount_<? echo $i; ?>" value="<? echo -(float) $valor_desconto / 100; ?>" /> -->
					...
					<input type="hidden" name="amount" value="<? echo (float) $valor_final; ?>">

					<input type="submit" class="submit cielo" value="Pagar com PayPal" />
...
<section id="overlay-cupom">
	...
	<article>
		...
		<a href="<? echo SITE.$link_lang; ?>ingressos/pagamento/paypal/<? echo $cod; ?>/remover/<? echo $cupom_apenas_folia; ?>/" class="sim ctrl">Sim</a>
		<? } else { ?>
