...
$sql_cliente = sqlsrv_query($conexao_sankhya, "SELECT TOP 1 * FROM TGFPAR WHERE CODPARC='$usuario_cod'", $conexao_params, $conexao_options);
	...
	if(sqlsrv_num_rows($sql_loja) > 0) {
		...
		<section class="padding">
			...
			<section id="financeiro-lista" class="secao label-top">
				<table class="lista">
					<thead>
						<tr>
							<th class="first"><strong><? echo $lg['pagamento_compra_voucher']; ?></strong></th>
							<th><strong><? echo $lg['pagamento_compra_cliente']; ?></strong></th>
							<th><strong><? echo $lg['pagamento_compra_tipo']; ?></strong></th>
							<th><strong><? echo $lg['pagamento_compra_dia']; ?></strong></th>
							<th><strong><? echo $lg['pagamento_compra_setor']; ?></strong></th>
							<th class="right"><strong><? echo $lg['pagamento_compra_valor']; ?></strong></th>
						</tr>
		...
		<section id="compra-pagamento">
			...
			<section id="cupom-parceiro">
				<form class="controle" id="form-cupom-parceiro" action="<? echo SITE.$link_lang; ?>ingressos/pagamento/v2/cupom-parceiro/" method="post">
					...
					<p>
						<label for="compra-parceiro" class="infield"><? echo $lg['pagamento_compra_codigo_parceria']; ?></label>
			...
			<section id="cupom-pagamento">
				<? if(!$loja_desconto) { ?>
					...
					<form class="controle" id="form-cupom-pagamento" action="<? echo SITE.$link_lang; ?>ingressos/pagamento/v2/cupom/" method="post">
						...
						<p>
							<label class="infield" for="compra-cupom"><? echo $lg['pagamento_compra_cupom_desconto']; ?></label>
	        ...
	        <form class="form-padrao controle" id="form-compra-pagamento" action="<? echo SITE.$link_lang; ?>ingressos/pagamento/cielo/<? echo $loja_cod; ?>/" method="post">
	    		...
	    		if(sqlsrv_num_rows($sql_item) > 0) {
	    			...
	    			while ($item = sqlsrv_fetch_array($sql_item)) {
	    				...
	    				<input type="hidden" name="cart_<? echo $i; ?>_type" value="1" />
	    				<?
	   ...
	   //  		echo var_dump(empty($cliente_data_nascimento))."</br>";
				...
				if(!empty($cliente_data_nascimento)&&!empty($cliente_ddi)&&!empty($cliente_ddd)&&!empty($cliente_telefone)&&($cliente_pais == 'BR'&&(!empty($cliente_cpf_cnpj)&&($cliente_pessoa=="F"&&validaCPF($cliente_cpf_cnpj))||($cliente_pessoa=="J"&&validaCNPJ($cliente_cpf_cnpj))))||($cliente_pais != 'BR' && !empty($cliente_passaporte))){ ?>
	    			<input type="submit" class="submit cielo" value="<? echo $lg['pagamento_compra_realizar_pagamento']; ?>" />
	    		<?php }else{ ?>
					<input type="button" class="submit cielo" onclick="completarDados()" value="Realizar pagamento" />
	    		<?php }?>
...
<script>
	function completarDados(){
		swal({
			...
			text: "Sua informação de Data de Nascimento,CPF,CNPJ,DDI,DDD,Telefone ou Passaporte está vazia ou é inválida.",
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#74bb3d",
			...
			function(isConfirm){
				if (isConfirm) {
					window.location="<?php echo SITE.$link_lang."meus-dados/" ?>";
				}
			}
		);
	}
