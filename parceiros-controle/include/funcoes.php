...
function maskCartao ($string)
{
    switch (strlen($string)) {
        case '15': $replace = '\1*****\2'; break;        
        case '16': default: $replace = '\1******\2'; break;
    }

    $mask = preg_replace( '/^(......).*(....)$/', $replace, $string);
    return $mask;
}
...
// Função para alterar o lote
function lote($ingresso) {
    
    global $conexao, $conexao_params, $conexao_options;

    //Buscar estoque
    $sql_ingressos = sqlsrv_query($conexao, "SELECT *,
    CASE WHEN VE_LOTE IS NOT NULL THEN VE_LOTE_ATIVO ELSE 1 END AS ATIVO,
    CASE WHEN VE_LOTE IS NOT NULL THEN 1 ELSE 0 END AS LOTE
    FROM vendas WHERE VE_COD='$ingresso' AND D_E_L_E_T_=0", $conexao_params, $conexao_options);
    ...
    if(sqlsrv_num_rows($sql_ingressos) > 0) {
        ...
        $ingressos_lote_ativo = (bool) $ingressos['ATIVO'];
        $ingressos_lote_controle = (bool) $ingressos['LOTE'];

        // Se o ingresso respeita o sistema de lotes
        if($ingressos_lote_controle) {
            
            //Buscar comprados
            $sql_comprados = sqlsrv_query($conexao, "SELECT li.LI_INGRESSO, COUNT(li.LI_COD) as QTDE 
            FROM loja_itens li, loja l 
            WHERE li.LI_INGRESSO='$ingresso' 
            AND l.LO_COD=li.LI_COMPRA 
            AND l.D_E_L_E_T_=0 
            AND li.D_E_L_E_T_=0 
            GROUP BY li.LI_INGRESSO", $conexao_params, $conexao_options);
            ...
            if(!$disponibilidade) {
                ...
                $sql_update = sqlsrv_query($conexao, "UPDATE TOP (1) vendas SET VE_LOTE_ATIVO = 0 WHERE VE_COD ='$ingresso'", $conexao_params, $conexao_options);

                // Vamos buscar todos os lotes acima do atual e verificar qual está disponivel para ativar

                $sql_semelhantes = sqlsrv_query($conexao, "SELECT v1.*
                FROM vendas v1
                    JOIN vendas v2 
                    ON v1.VE_EVENTO = v2.VE_EVENTO
                    AND v1.VE_TIPO = v2.VE_TIPO
                    AND v1.VE_SETOR = v2.VE_SETOR
                    AND v1.VE_DIA = v2.VE_DIA
                    AND ISNULL(v1.VE_FILA, '') = ISNULL(v2.VE_FILA, '')
                    AND ISNULL(v1.VE_VAGAS, '') = ISNULL(v2.VE_VAGAS, '')
                    AND ISNULL(v1.VE_TIPO_ESPECIFICO, '') = ISNULL(v2.VE_TIPO_ESPECIFICO, '')
                WHERE v2.VE_COD ='$ingresso' AND v1.VE_LOTE > v2.VE_LOTE", $conexao_params, $conexao_options);

                if(sqlsrv_num_rows($sql_semelhantes)) {
                    while($semelhantes = sqlsrv_fetch_array($sql_semelhantes)) {

                        $semelhante_cod = $semelhantes['VE_COD'];
                        $semelhante_estoque = $semelhantes['VE_ESTOQUE'];

                        // Verificar se há em estoque produtos suficientes, senao pulamos já para o proximo lote

                        $sql_comprados = sqlsrv_query($conexao, "SELECT li.LI_INGRESSO, COUNT(li.LI_COD) as QTDE 
                        FROM loja_itens li, loja l 
                        WHERE li.LI_INGRESSO='$semelhante_cod' 
                        AND l.LO_COD=li.LI_COMPRA 
                        AND l.D_E_L_E_T_=0 
                        AND li.D_E_L_E_T_=0 
                        GROUP BY li.LI_INGRESSO", $conexao_params, $conexao_options);

                        $semelhantes_comprados = 0;

                        if(sqlsrv_num_rows($sql_comprados)) {
                            $comprados = sqlsrv_fetch_array($sql_comprados);
                            $semelhantes_comprados = (int) $comprados['QTDE'];
                        }
                        
                        $disponibilidade = (($semelhante_estoque - $semelhantes_comprados) > 0) ? true : false;
                    ...
                    if($semelhante_cod) {
                        ...
                        $sql_update = sqlsrv_query($conexao, "UPDATE v1
                        SET v1.VE_LOTE_ATIVO = 0
                        FROM vendas v1
                            JOIN vendas v2 
                            ON v1.VE_EVENTO = v2.VE_EVENTO
                            AND v1.VE_TIPO = v2.VE_TIPO
                            ...
                            AND ISNULL(v1.VE_VAGAS, '') = ISNULL(v2.VE_VAGAS, '')
                            AND ISNULL(v1.VE_TIPO_ESPECIFICO, '') = ISNULL(v2.VE_TIPO_ESPECIFICO, '')
                        WHERE v2.VE_COD ='$semelhante_cod'", $conexao_params, $conexao_options);

                        $sql_update = sqlsrv_query($conexao, "UPDATE vendas SET VE_LOTE_ATIVO=1 WHERE VE_COD ='$semelhante_cod'", $conexao_params, $conexao_options);
...
function enviarEmail($remetente_nome, $remetente_email, $destinatario_nome, $destinatario_email, $assunto, $mensagem, $reply_nome=null, $reply_email=null) {
    try {
        ...
        $mail->AddCC("marcos@fococomunicacao.com", "Foco Comunicacao");
